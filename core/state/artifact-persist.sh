...
_artifact_persist_snapshot() {
    ...
    [[ -z "$repo_root" || ! -d "$repo_root/.git" ]] && return 0
    ...
    while IFS= read -r -d '' f; do
        ...
        blob="$(GIT_DIR="$repo_root/.git" git hash-object -w "$f" 2>/dev/null)" || { rc=1; break; }
        GIT_INDEX_FILE="$tmp_index" GIT_DIR="$repo_root/.git" \
    ...
    for extra in scope-manifest.md intake.md; do
        ...
        blob="$(GIT_DIR="$repo_root/.git" git hash-object -w "$state_dir/$extra" 2>/dev/null)" || continue
        GIT_INDEX_FILE="$tmp_index" GIT_DIR="$repo_root/.git" \
    ...
    tree="$(GIT_INDEX_FILE="$tmp_index" GIT_DIR="$repo_root/.git" git write-tree 2>/dev/null)" \
        || { rm -f "$tmp_index"; return 1; }
    rm -f "$tmp_index"

    parent="$(GIT_DIR="$repo_root/.git" git rev-parse -q --verify "refs/heads/$branch" 2>/dev/null || true)"
    # Skip an empty commit when the tree is identical to the current tip.
    if [[ -n "$parent" ]]; then
        local parent_tree; parent_tree="$(GIT_DIR="$repo_root/.git" git rev-parse -q --verify "$parent^{tree}" 2>/dev/null || true)"
        [[ "$parent_tree" == "$tree" ]] && return 0
    fi

    local msg="zbuild: persist artifacts for #$issue [skip ci]"
    if [[ -n "$parent" ]]; then
        commit="$(GIT_DIR="$repo_root/.git" git commit-tree "$tree" -p "$parent" -m "$msg" 2>/dev/null)" || return 1
    else
        commit="$(GIT_DIR="$repo_root/.git" git commit-tree "$tree" -m "$msg" 2>/dev/null)" || return 1
    fi
    GIT_DIR="$repo_root/.git" git update-ref "refs/heads/$branch" "$commit" 2>/dev/null || return 1
    return 0
}
...
_artifact_persist_restore() {
    ...
    local branch; branch="$(_artifact_persist_branch "$issue")"
    local ref=""
    if GIT_DIR="$repo_root/.git" git rev-parse -q --verify "refs/heads/$branch" >/dev/null 2>&1; then
        ref="refs/heads/$branch"
    elif GIT_DIR="$repo_root/.git" git rev-parse -q --verify "refs/remotes/origin/$branch" >/dev/null 2>&1; then
    ...
    GIT_DIR="$repo_root/.git" git archive "$ref" 2>/dev/null | tar -x -C "$restored_dir" 2>/dev/null || return 1
    return 0
}
