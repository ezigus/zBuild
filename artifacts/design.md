...
3. Update unit and integration tests that assert on `--dangerously-skip-permissions`, including the stale `_args` fixture in `stage-checkpoint-test.sh` (line 233).
4. Amend ADR-018 to record the permission-mode change.

The settings file is built before `_zbuild_make_fresh_shell` runs (mirroring the restate-before-scrub pattern). If `ZBUILD_STAGE_SCRATCH` is unset, the helper falls back to `zbuild_engine_tmpdir()` and emits a warning. The `permissions.sh` source in `route.sh` is conditional (not `_zbuild_route_require`) so the stub-tree load test works without a fixture; spawn sites fail closed (command not found) if the file is absent.
...
SPEC-4[change]: grep -rn dangerously-skip-permissions core/router/ returns no results (comments excluded)
