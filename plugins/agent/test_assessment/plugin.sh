...
$_ta_instructions"
    ...
    # shellcheck disable=SC2034  # _ta_prose is a required output-param of
    # _llm_envelope_parse; test_assessment emits no prose sidecar (only impact does).
    ...
    # entirely and emit the LLM verdict as-is. simple.yaml uses the gate-aggregator
    # as its convergence driver; test_assessment is advisory-only there and must not
