...
description: |
  ...
  An unresolvable baseline yields NO diff files. That is reported as
  skip `no_baseline` (#2064), NOT `no_shape_change` — the floor still degrades
  open and invents no verdict, but it says it could not check rather than
  claiming it checked and found nothing.
