...
- merge: no `provides:` block; `valid_verdicts: []` (empty) in config; no events declared;
  plugin.sh has `return 2` on missing-state-file, branch-is-main, checkout, push, gh-create,
  and gh-merge paths.
- pr-open: has `provides.events` (two events); `valid_verdicts: []` (empty) in config; no role
  (explicitly prohibited by manifest comment: "Do not give it a role"); plugin.sh has `return 2`
  on multiple paths.
...
5. `review.json` (merge) and `review-report.json`/`review.json` (pr-open) BY-PATH reads remain
...
TDD order per plugin: write v2 result test → confirm red → implement → confirm green.
...
`plugins/agent/deploy/tests/deploy-test.sh` — SPEC-7 asserts `schema_version=1` on the dry-run
path. The dry-run path in `deploy_release_run` writes `result_contract:2` after our migration;
the assertion fails. In scope to update SPEC-7 to check `result_contract=2`.
...
SPEC-9[change]: deploy-result.json carries result_contract:2, verdict, disposition, and non-empty reason on every exit path (dry_run, tag_success, tag_failed, push_failed)
