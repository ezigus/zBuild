...
**`tests/unit/summary-producers-test.sh`** — lines 51–54 assert that `acceptance_detail` is
declared as the stage's summary and that its `required` field is `"true"`. Both assertions pin
the manifest shape this change introduces. In scope because removing the change causes these
assertions to fail.

**`tests/unit/summary-mandatory-test.sh`** — SPEC-1 sweeps `plugins/*/*/manifest.yaml` and
fails if any stage-bound plugin has zero `summary: true` outputs. Before this migration,
spec-acceptance had no such output, so spec-acceptance would appear in the `_missing` set.
After this change, `acceptance_detail` carries `summary: true` and the assertion passes for
spec-acceptance. In scope because the test gates on the manifest change.
...
`required: true` and `summary: true`, and the input declaration with no stale `source:` form.
