#!/usr/bin/env bash
# Tests: bounded parallel execution in scripts/run-tests.sh (issue #983, EPIC #982).
#
# #983 makes parallel test execution safe by construction:
#   - opt-in via ZBUILD_TEST_PARALLEL_JOBS (default 0 = serial),
#   - tier-gated to a safe allow-list (ZBUILD_PARALLEL_SAFE_TIERS, default "unit")
#     so non-safe tiers (e.g. integration, until #991) stay serial even when JOBS
#     is set — this is the fix for the fork-bomb that deadlocked the integration
#     tier's route.sh/claude-spawning tests when run concurrently,
...
_run_tier() {
  ...
  env ZBUILD_TESTS_DIR="$FAKE_TESTS" \
