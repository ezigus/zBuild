...
## Amendment v2 (2026-06-14, #842) — discovery contract: design owns scope, impact finds consequences

The scope-governance model requires an **upstream scope enumeration** that is exhaustive before `build` expands. With `plan_impact_cycle`, plan's `steps[].files[]` was the baseline — a strategic prediction made before the implementation exists. This Amendment formalises the two-layer discovery contract:

**Layer 0 — Exhaustive enumeration (design stage, ADR-841 mandate):** design actively searches the repo (Read/Grep/Glob) and produces a `\`\`\`scope` fenced block covering every file the change touches, invalidates, references, validates, documents, or assumes. The scope block is the authoritative write-scope baseline that `build` and the resolver operate against. plan.json's `steps[].files[]` is a seed hint; design's scope block supersedes it.

**Layer 0.5 — Post-design adversarial check (impact stage):** impact reads design.md's scope block and adversarially finds **post-design consequences** — files the change affects that design missed. Impact is NOT re-enumerating from plan.json; it is challenging design's already-comprehensive scope block. This is consequence-finding, not symbol-grep from a file list.

**Cycle topology change (`design_impact_cycle`):** `plan_impact_cycle` (plan→impact, 5 iter) is replaced by `plan` (leaf) + `design_impact_cycle` (design→impact, 3 iter). The cycle feedback edges mirror #773's lesson: impact gap-report → `design.prior_impact_feedback` (so design expands its scope block); design self-edge → `design.prior_design` (so design refines rather than re-creates). plan remains a leaf; it no longer receives cycle feedback, which simplifies its prompt and removes the re-plan regression risk (#773).

**Deterministic floor unchanged:** the `_impact_scope_prefilter` (shape-change-golden + shape-change-numeric CLAUDE.md mandates) continues to run in impact, using plan.json as secondary input. The floor is not weakened by the primary-source change.

## Amendment v3 (2026-06-14, #879 / REC-1) — build requests needed collateral on verdict=pass; honesty

The #842 dogfood exposed a recovery gap orthogonal to v1's *created*-collateral lane. Build emitted a `scope_expansion_request` only when it produced **no** in-scope diff (`empty_diff`, Path A) or **created** a file (Path B). When build did **valid in-scope work** (`verdict=pass`) but the full suite was still red because an *existing* out-of-scope file (e.g. a test pinning an old value) needed changing, there was **no channel** to request it: build converged a clean `pass` while the suite was red, and the cycle ground to `max_iterations`.

- **Build asks even on `verdict=pass` (extends Layer 3).** A new branch (`_build_pending_collateral_request`) fires when `verdict=pass` AND the prior-assessment feedback names out-of-scope files the change requires. It reuses the Layer-3 detector + evidence machinery (`_build_detect_out_of_scope_files` + `_build_scope_expansion_request`): collateral classes carry a verified evidence token and are auto-grantable; source files are classified `structural` and the resolver denies/escalates them (build only *constructs* requests — the floor is enforced by the resolver, unchanged). Mutually exclusive with Paths A/B.
- **Honesty signal.** When this branch fires, the build summary carries `reason=scope_request_pending` + `out_of_scope_files[]`. Build does not report a clean terminal "complete" while it has identified out-of-scope files it has not requested. The cycle stays non-converged (the #847 `test_assessment` guard already forbids `verdict=pass` while `test.failed>0`), so the request is resolved between iterations: grant → next iter fixes the file → converge; deny → `blocked_on_scope` clean abandon. Never a loop.
- **Stage-agnostic.** As in v1, the request/resolve mechanism is not build-specific; the `_build_pending_collateral_request` shape is the template for future file-mutating agent-loop stages (cq-cycle, cq-backtrack).

## Amendment v4 (2026-07-06, #1265) — census baselines against the intake untracked snapshot

The scope-census swept the *live* working tree (`git add -N .` → `git diff --name-status HEAD`), so a **pre-existing untracked stray** — a file left in the tree by a PRIOR run, present before this run's intake, never created or touched by *this* build — was intent-added, surfaced as a status-A path, classified out-of-scope collateral, and raised `build.scope.violation` (`created: true`). That single unrelated stray then zeroed the *entire* `diff.patch` and skipped the commit, discarding the legitimate in-scope work too (`#1214` dogfood, run `20260706155412-97543`; the same stray that derailed `#945`-run-1). The census must only judge **this run's delta**.

- **Intake snapshots the pre-existing untracked set (run-start ground truth).** `intake` writes `${state_dir}/intake-untracked-baseline.txt = git ls-files --others --exclude-standard -z` and emits `intake.untracked_baseline.captured count=N`. Captured **unconditionally** — even under `ZBUILD_INTAKE_ALLOW_DIRTY=1` (the dogfood path where strays enter), which is exactly when the snapshot is needed.
- **Build's census excludes pre-existing untracked paths.** Build loads the snapshot (`_build_load_preexisting_untracked`, materialized to a sorted temp for bash-3.2-safe `grep -Fxq`) and (a) does a **selective `git add -N`** that intent-adds only untracked paths *absent* from the snapshot — a stray never enters `diff.patch` or the census; (b) as defense-in-depth, the census loop `continue`s (no violation, no `scope_violations_created`) for a status-A path in the snapshot. Fallback: no baseline file (resumed / non-intake run) → legacy blanket `git add -N .`.
- **Pre-existing untracked ≠ build-created collateral.** A path in the intake snapshot is *out of judgment*, distinct from the v1 build-*created* (status-A, absent from the snapshot) collateral lane, which STILL raises `build.scope.violation` + `scope_violations_created` and flows through the governed grant/deny resolver unchanged. This is a path-set exclusion ("didn't create this run"), not a content-hash refinement (deferred — a stray the build *modifies* is a rare edge, out of scope for #1265).
- **Interactions preserved.** The `#827` path (revert only OOS, preserve in-scope diff — since #2185 on every scope violation, not only `router_rc >= 2`; see the #2185 amendment) is unaffected — the census skip only removes pre-existing strays from the OOS set. The `#870`/`#840` governed expansion is unchanged.

## Amendment v5 (2026-09-24, #2185) — a scope violation keeps the in-scope work, whatever stopped the router

Layer 3 already said build emits its request "alongside its in-scope diff". The code kept the in-scope diff only on the `#827` path, keyed on `router_rc >= 2`, and zeroed the diff and skipped the commit otherwise. Since #1208 the router returns **0** on a timeout and reports it through `_ROUTE_LOOP_TERMINATED_REASON=router_timeout`, so the `#827` branch could never fire: the #1849 run (Actions 35949629759) lost a 45-minute build attempt to one out-of-scope doc edit.

- **Every scope violation** reverts only the out-of-scope paths (restored from HEAD, or removed with their intent-to-add entry when build created them), keeps the in-scope diff, and **commits it** — `commit.sh` stages plan files only. Build no longer reads the router's rc for this.
- **The violation stays reported**: verdict `scope_violation`, `scope_violations[]`, and the `scope_expansion_request` the cycle resolves as before.
- Event `build.timeout.partial_work_preserved` is replaced by `build.scope.inscope_preserved`.
