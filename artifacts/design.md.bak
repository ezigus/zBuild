...
**Router budgets.** All three are T0 tools with no LLM calls. Reference v2 T0 manifests (mutation-gate, shape-floor) carry no `router:` section; `tier_default: T0` is the complete and correct budget declaration for LLM-free tools. All three manifests already declare `tier_default: T0` — no `router:` block is added. **Primary outputs.** All three manifests already declare `primary: true` on their canonical result artifact (`merge_result`, `pr_url`, `deploy_result`). The merge manifest migration (adds `provides:`) must not disturb that declaration; pr-open and deploy-release retain theirs unchanged.
...
SPEC-22[guard]: all three manifests declare tier_default: T0 with no router: block — absence of router: is the correct budget declaration for LLM-free T0 tools
SPEC-23[guard]: all three manifests retain primary: true on their canonical result output (merge_result, pr_url, deploy_result) after the v2 migration
...
SPEC-22: tests/unit/merge-v2-result-test.sh tests/unit/pr-open-v2-result-test.sh tests/unit/deploy-release-v2-result-test.sh
SPEC-23: tests/unit/merge-v2-result-test.sh tests/unit/pr-open-v2-result-test.sh tests/unit/deploy-release-v2-result-test.sh
```
...
- `tests/integration/merge-policy-auto-test.sh` — asserts `.status == "merged"` and `.status == "pr_fallback"` on merge-result.json
...
**pr-open SPEC-18 rc clarification.** After v2 migration, review-block and no-signal paths return rc=0 (not rc=2) but write `verdict=blocked`. Existing assertions in `pr-open-test.sh` and `pr-pipeline-test.sh` that expect rc=2 on block must be updated to expect rc=0 with `verdict=blocked`.

**SPEC-22/23 guard rationale.** Both behaviors are currently true at the merge-base. SPEC-22 guards that the manifest migration (adding `provides:` to merge, updating `provides:` in pr-open and deploy-release) does not inadvertently introduce a `router:` block. SPEC-23 guards that the merge manifest's new `provides:` section does not displace or corrupt the `outputs[].primary: true` declaration already present.
