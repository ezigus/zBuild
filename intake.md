[Phase 0/F] migrate the test plugin to contract v2

Part of #1819 (Phase 0 — the stage↔engine contract). Member of the **F set** — one plugin per PR, each independently verifiable.

Migrate `plugins/tool/test` to contract v2. The engine reads v1 and v2 side by side (#1824), so this plugin moves on its own and nothing else has to move with it.
...
The test plugin is where the contract's absence is most expensive, and three open issues land here:

- **#1747** — a SIGTERM'd test stage is classified `verdict=error` and terminates the cycle. It becomes one line: `disposition: interrupted`. **The nuance must survive the migration:** only a *signalled* death or a true timeout is recoverable. An rc=0 exit with unrecognised output is a real config bug and must remain a structural failure.
- **#1748** — the plugin spawns without a process group and its subprocess tree outlives the run by 15+ minutes. This becomes **the first real implementation of `release`** (#1829): a `setsid`-wrapped spawn plus a TERM trap, reusing `_ROUTE_PG_PREFIX` from `core/router/route.sh:245` (#687) rather than reinventing it.
- `plugins/tool/test/plugin.sh:211` `rm -rf`s the staging directory from a RETURN trap. Under `release` it must delete nothing — that is the inversion #1829 exists to fix, and the test for it is: kill a run mid-`test`, assert no surviving process group **and** the staging tree still on disk.

Coordinate with #1772 (a single-tier run with zero discovered test files exits 0) and #1784 (`tree_sha` recorded for suite reuse and never read) — both are things the v2 `data` block should carry explicitly.
