...
**The system temp is swept only while §3's redirect is in effect.** A file in `/tmp` is attributable to a stage only when the engine has pointed `TMPDIR` somewhere else for that dispatch. Where the redirect is not in scope, `ZBUILD_STAGE_SCRATCH` is unset and the engine's own temps — the template merge, the redaction buffers, the router captures — land in the system temp legitimately, alongside whatever else is running on the machine; sweeping it then halts the run on the engine doing its job (observed on ubuntu CI as `stage=intake path=/tmp/zb-route-redact-out.*`). `write_boundary_watch_list` therefore drops the system-temp roots when scratch is unavailable. **Exact roots only, never a prefix** — a watch entry that merely lives *under* the system temp is a deliberate target and is still swept. The alternative, allowlisting engine temp filenames, would have to grow forever and would let a stage evade the boundary by choosing its name. `write-boundary-sweep-test.sh` SPEC-4g pins both directions.
