...
lint_gate_run() {
    ...
    if [[ -n "$state_file" && -d "$(dirname "$state_file")" ]]; then
        ...
        artifacts_dir="${ZBUILD_ARTIFACT_DIR:-$(zbuild_engine_tmpdir)/zbuild-lint-gate-artifacts}"
    fi
    ...
    jq -n --arg v "$verdict" --arg s "$status" --arg d "$detail" \
        '{"result_contract":2,"verdict":$v,"disposition":"complete","reason":$d,"status":$s,"detail":$d}' \
