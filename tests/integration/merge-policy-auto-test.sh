...
# verdict==specification (≠pass). The merge guard (verdict != pass → PR path) must
# treat it exactly like any non-pass verdict: NEVER auto-merge, fall back to a PR.
# This guards the invariant that a specification in-flight (the bounded rewind is
# handled by the runner, not merge) can never ship to main.
print_test_section "SPEC-8: merge_policy==auto + gate verdict==specification → PR fallback"

_sf8="$(_setup_run s8 specification)"
...
assert_eq "[SPEC-8] merge-result.json status==pr_fallback on specification" \
