...
- **Interactions preserved.** The `#827` timeout path (`router_rc >= 2`: revert only OOS, preserve in-scope diff) is unaffected — the census skip only removes pre-existing strays from the OOS set. The `#870`/`#840` governed expansion is unchanged.
