...
assert_eq "[SPEC-11] T5: parallel group recorded in stage_statuses (cycle-member path)" "complete" "$gates_ss"
gates_sv="$(jq -r '.stage_verdicts.gates // "missing"' "$STATE_FILE")"
assert_eq "[SPEC-12] T5: parallel group recorded in stage_verdicts (cycle-member path)" "pass" "$gates_sv"
