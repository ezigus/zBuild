...
_pr_stage_run_inner() {
    ...
    if [[ "${ZBUILD_DRY_RUN:-0}" == "1" ]]; then
        ...
        printf '{"status":"dry_run","branch":"%s","pr_number":0,"draft":%s}\n' \
            "${ZBUILD_BRANCH:-unknown}" "$_dry_draft" | atomic_write "$pr_result_out"
        return 0
    fi
    ...
    if pr_url="$(gh pr create "${_fb_gh_args[@]}" 2>/dev/null)"; then
        ...
        printf '{"status":"opened","branch":"%s","pr_url":"%s","draft":%s}\n' \
            "${branch}" "$pr_url" "$_fb_draft" | atomic_write "$pr_result_out"
    else
