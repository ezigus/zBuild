...
# Resolve the ledger the way the router does (${ZBUILD_COST_LEDGER:-$HOME/…})
# so we read/write the same path whether or not the test-stage fence set it
# (ADR-024: tests must honor the fence vars, never hardcode $HOME/.zbuild/*).
LEDGER="${ZBUILD_COST_LEDGER:-$HOME/.zbuild/cost-ledger.jsonl}"
...
ledger_lines="$(wc -l < "$LEDGER" 2>/dev/null | tr -d ' ' || echo 0)"
