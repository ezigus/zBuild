...
| Sev | File(s) | Issue | Fix |
|-----|---------|-------|-----|
| MED | 9 mock-`claude` tests: `loop-auto-terminate-no-progress`, `router-loop-preserves-error-artifacts`, `build-test-cycle-progress`, `build-no-stash-flow`, `router-loop-rc124-honors-sentinel`, `loop-already-done-emits-complete`, `build-changed-files-summary`, `build-banner-llm-prompt-visible`, `build-loop-banner` | Mock `claude` body bakes `mark="${MARK_FILE:-/tmp/mark}"` — a shared fixed fallback reached if `MARK_FILE` isn't inherited (env-strip / refactor). | Interpolate the resolved path into the mock body at write time (`mark="$MARK_FILE"`) + assert `MARK_FILE` non-empty at setup; drop the `/tmp/mark` fallback. |
