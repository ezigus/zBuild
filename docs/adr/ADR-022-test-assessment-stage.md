...
- **branch_numstat helper**: lived in `scripts/lib/helpers.sh`; fail-OPEN with fallback chain (origin/main → main → master → HEAD~50 → HEAD~1 → "unknown"). NOT a safety gate — LLM signal only. *(Removed in #1655, along with its `branch_numstat_since` sibling from #824: this stage was their only consumer and it retired with the lattice, so both had been dead since. Their fallback chain is the same guess #1655 removed from the baseline resolver — a second copy of it in the tree was worth deleting rather than leaving to be copied from.)*
