...
# ADR refs: ADR-001 (plugin contract), ADR-003 (tier), ADR-004 (redaction), ADR-013 (stage list)
...
_review_stage_run_inner() {
    ...
    if [[ -f "$test_results_json_path" ]]; then
        ...
        warn "review_stage_run: test-results.json not found at $test_results_json_path; treating as test failure"
    ...
    local prompt
    prompt="$(cat <<PROMPT
Review whether this diff implements the plan. Respond with a JSON object only — no prose, no code fences.
