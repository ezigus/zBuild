...
description: |
  ...
  pure grep over design.md + the red-first stubs design already writes, with NO
  ...
  whack-a-mole):
    ...
    C5 WIRING          — a WIRING: section is present ("none" ok); each concrete
                         path exists on disk.
    C6 LEVEL-1 TAGS    — every SPEC-n has a [SPEC-n] tag in a declared testfile
                         (acceptance-coverage.sh, shifted left from acceptance-gate).
...
hooks:
  init: design_gate_init
  run: design_gate_run
  finalize: design_gate_finalize
