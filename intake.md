[Phase 0/F] migrate the review-report plugin to contract v2

Part of #1819 (Phase 0 — the stage↔engine contract). Member of the **F set** — one plugin per PR, each independently verifiable.

Migrate `plugins/agent/review-report` to contract v2. The engine reads v1 and v2 side by side (#1824), so this plugin moves on its own and nothing else has to move with it.
...
Reporting is the stage most damaged by the five-channel problem: findings sit in `review-report.json` while the PR body says something else (#1707, landed). Under #1825 the report declares its output once and the PR stage references it by `from:` rather than re-deriving a path.

Coordinate with #1719 (review-summary comment count is cumulative, and the verdict marker it falls back from is never emitted).
...
This migration opens `review-report`. Two other pieces of work touch the same file, and
doing them here is one pass instead of three.

### 1. The shared reply parser (#2035) — a live defect

`plugins/agent/review-report/lib/lenses.sh:145` still reads the model's reply with bare
