...
# ── SPEC-2: all 16 expected mechanic names are present ───────────────────────
EXPECTED_MECHANICS=(
    admission-gate
    aggregators
    convergence
...
assert_eq "[SPEC-2] all 16 expected mechanics present in registry (missing: ${missing_mechanics[*]:-none})" "0" "$missing_count"
