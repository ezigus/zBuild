...
Automatically tunes daemon parameters based on DORA metric thresholds. Enabled by default in the shipped daemon config.

| Field               | Default | Description                                               |
| ------------------- | ------- | --------------------------------------------------------- |
| `self_optimize`     | `true`  | Enable self-optimizing loop (includes context efficiency) |
...
| `intelligence.optimization_enabled` | `true`  | Enable self-optimization (outcome analysis, template tuning, model A/B testing) |
| `intelligence.prediction_enabled`   | `true`  | Enable predictive analytics (risk assessment, anomaly detection, AI patrol)     |
...
| Field                            | Default | Description                                               |
| -------------------------------- | ------- | --------------------------------------------------------- |
| `intelligence.ab_test_ratio`     | `0.2`   | Fraction of runs using composed pipelines for A/B testing |
...
The decision engine handles autonomous operational decisions with risk-tiered approval. Configure in `config/policy.json` under the `decision` key.

| Field                               | Default                      | Description                                          |
| ----------------------------------- | ---------------------------- | ---------------------------------------------------- |
| `decision.enabled`                  | `true`                       | Enable the autonomous decision engine                |
| `decision.cycle_interval_seconds`   | `1800`                       | Seconds between decision cycles                      |
| `decision.tiers_file`               | `config/decision-tiers.json` | Risk tier definitions for decisions                  |
| `decision.outcome_learning_enabled` | `true`                       | Learn from decision outcomes to improve future calls |
| `decision.outcome_min_samples`      | `10`                         | Minimum samples before applying learned adjustments  |
| `decision.dedup_window_days`        | `7`                          | Window for deduplicating identical decisions         |

## Context Engineering Configuration

Context engineering controls how pipeline agents compose and trim their context window. Configure in `config/defaults.json` under the `loop` key.

| Field                             | Default  | Description                             |
| --------------------------------- | -------- | --------------------------------------- |
| `loop.context_budget_chars`       | `180000` | Maximum characters for composed prompts |
| `loop.context_trim_memory_chars`  | `20000`  | Max characters from memory injection    |
| `loop.context_trim_git_entries`   | `10`     | Max git log entries included in context |
| `loop.context_trim_hotspot_files` | `5`      | Max hotspot files included              |
| `loop.context_trim_test_lines`    | `50`     | Max lines of test output included       |

The self-optimization loop monitors context efficiency events (`loop.context_efficiency`) and recommends budget adjustments automatically. See the [Intelligence guide](/guides/intelligence/#context-efficiency-closed-loop) for details.
