...
# ADR-051 §4 (#1305): per-stage `persona:` binding — the persona id that resolves
# for this stage. Read LAZILY from the loaded template source (like
# template_stage_negctl_timeout), so it needs NO row-shape / parser-array change.
# Matches the stage in all three template shapes (top-level section, inline list
# item, stage_definitions sub-entry) then reads the direct `persona:` sub-key.
# Returns empty when unset; no validation (persona id is an opaque string).
...
# ADR-051 §4 (#1305): top-level `config.persona` — the global template default
# persona id, applied when no per-stage binding is set for a stage. Read LAZILY
# from the loaded template source. Returns empty when unset.
