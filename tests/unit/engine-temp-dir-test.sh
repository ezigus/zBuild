#!/usr/bin/env bash
# Tests: zbuild_engine_tmpdir — one definition of "where engine code writes a
# temporary file" (#2010, deduped in #2017).
#
...
print_test_header "zbuild_engine_tmpdir — one definition, no \${TMPDIR} leak (#2010)"
...
_s1="$(ZBUILD_STAGE_SCRATCH="$SCRATCH" TMPDIR="$FAKE_TMP" ZBUILD_DATA_ROOT="$DATA_ROOT" \
        bash -c 'source "'"$REPO_ROOT"'/scripts/lib/helpers.sh"; zbuild_engine_tmpdir')"
...
_s3d="$(env -u ZBUILD_STAGE_SCRATCH -u ZBUILD_STATE_ROOT \
        TMPDIR="$FAKE_TMP" ZBUILD_DATA_ROOT="$DATA_ROOT" \
        bash -c 'source "'"$REPO_ROOT"'/scripts/lib/helpers.sh"; d="$(zbuild_engine_tmpdir)"; mkdir -p "$d" && mktemp "$d/probe.XXXXXX"')"
if [[ -f "$_s3d" ]]; then
