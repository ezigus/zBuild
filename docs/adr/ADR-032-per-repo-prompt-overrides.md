...
- **OV-2 (#855)** wires the same loader into `intake`/`plan`/`impact`/`build`/`review`/`test_assessment` (pure call-site wiring, no loader change). **OV-3 (#856)** is the companion compiler/typecheck gate for statically-typed targets, where the type-checker is the deterministic absence-collateral catcher.
